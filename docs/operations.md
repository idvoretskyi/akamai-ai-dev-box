...
Run OpenTofu >= 1.10 on an external trusted administration host, not on the instance it manages. Use Linode provider `3.12.0` and the committed dependency lock file. Set provider auth from local `linode-cli` configuration:
...
| --- | --- |
| `username` | local `$USER` by default (override via `TF_VAR_username` or `$DEVBOX_USER`); developer account with sudo access |
...
Use `-reconfigure` only when deliberately switching to a different backend configuration (not for routine init).
...
Outputs `ssh_command`, `instance_id`, and `ipv4_address` identify the created instance and access command. They **do not assert readiness**. Check the SSH host fingerprint using a trusted channel before accepting it. Bootstrap intentionally interrupts SSH for two reboots: first for the distribution kernel, then for the selected NVIDIA driver.
...
1. Cloud-init installs the minimal development packages (`tofu/cloud-init/main.yaml.tpl`), creates the non-root user, and writes `/etc/ai-dev-box.env` plus the bootstrap service. The environment file carries pinned release versions/checksums, not provider or workstation credentials.
2. `ai-dev-box-bootstrap.service` runs as a root oneshot after `cloud-final.service`. It installs `linux-generic` and `linux-headers-generic`.
...
4. Using `ubuntu-drivers-common`, the service selects the current hardware-aware, distro-recommended compute driver package explicitly (via `ubuntu-drivers list --gpgpu --recommended`) and installs that exact package plus matching NVIDIA utilities. No driver branch is pinned. It records the current boot ID in `/var/lib/ai-dev-box/driver-boot-id` and requests the second reboot.
5. After the second reboot, working `nvidia-smi` and a loaded driver version >= 550 are required before proceeding. The service records the actual loaded driver version in the readiness journal, installs pinned Ollama and OpenCode release binaries (see `tofu/cloud-init/releases.json`) with SHA256 verification, then enables localhost Ollama.
...
Ubuntu package and security revisions follow apt, including the recommended compute-driver selection. Package availability at any point in time is not a driver pin, proof of which driver this GPU will select, or a GPU compatibility trial. Record the actual loaded driver (from the readiness journal) rather than inferring it from a package name.
